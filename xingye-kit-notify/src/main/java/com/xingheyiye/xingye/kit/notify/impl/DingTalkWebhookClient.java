...
/**
 * {@link WebhookClient} 的钉钉群机器人实现：发送 text 类型消息并按 errcode 判定结果。
 *
 * <p>一句话职责：把一段文本经
 * {@code WebhookJson.textPayload} 组包后 POST 到钉钉机器人地址，解析 errcode/errmsg。</p>
 *
 * <p>适用场景：部署通知、告警播报推送到钉钉群。注意：钉钉机器人的安全设置必须包含
 * "自定义关键词"（本实现发送的文本需包含该关键词）或"IP 白名单"（服务器出口 IP），
 * 或"加签"（本实现未实现加签，需加签请自行扩展），否则钉钉返回 errcode=310000 权限错误。</p>
 *
 * <p>线程安全性：仅持有不可变 webhookUrl 与静态预编译正则，线程安全，可共享单例。</p>
 *
 * <p>使用示例：</p>
 * <pre>{@code
 * WebhookClient client = new DingTalkWebhookClient(
 *         "https://oapi.dingtalk.com/robot/send?access_token=xxx");
 * SendResult result = client.send("部署通知", "上线通知：服务 v1.2.0 已发布");  // 文本需含机器人关键词
 * }</pre>
 *
 * @author 星河一叶 (RainxButterfly)
 * @since 2026-08-23
 */
public class DingTalkWebhookClient implements WebhookClient {
     ...
     * 构造钉钉 Webhook 客户端。
     *
     * @param webhookUrl 钉钉机器人 Webhook 地址（形如
     *                   https://oapi.dingtalk.com/robot/send?access_token=xxx），
     *                   不能为 null 或空白串
     * @throws IllegalArgumentException webhookUrl 为 null 或空白串时抛出
     */
    public DingTalkWebhookClient(String webhookUrl) {
     ...
     * 发送 text 类型消息到钉钉群。
     *
     * <p>判定规则：响应体中 {@code "errcode":0} 视为成功（msgId 为 "dingtalk"）；
     * errcode 非 0 时以 errcode 值为错误码、errmsg 为错误信息返回失败；
     * 网络异常（含超时、非 2xx）返回 {@code fail("NETWORK_ERROR", ...)}；
     * 响应体中未找到 errcode 字段返回 {@code fail("PARSE_ERROR", ...)}。</p>
     *
     * @param title 消息标题；钉钉 text 类型消息无标题字段，本实现忽略该参数
     * @param text 消息正文；为 null 时按空串发送；注意需包含机器人安全设置的关键词，
     *             且最长 20000 字节（UTF-8），超限由钉钉返回错误码
     * @return 发送结果，不会为 null；success 为 true 时 msgId 为 "dingtalk"
     */
    @Override
